...
task main() {
	const int MAX = 127; // max motor value for convenience

	for (int counter = 20; counter > 0; counter--) { // loop 20 times
		while (SensorValue[limit] != 1) { // wait for limit switch to be pressed
			motor[lMotor] = MAX / 2;        // while waiting, motor is half speed
		}
		motor[lMotor] = -MAX / 2; // rotate opposite direction
